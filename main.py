...
async def uploader_handler(client, message):
    chat_id = -1001234567890  # Replace with your channel ID (must be forum-enabled)

    topic_title = f"📦 Upload from {message.from_user.first_name} - {datetime.now().strftime('%d %b %H:%M')}"
    topic = await client.create_forum_topic(chat_id=chat_id, name=topic_title)

    await client.send_message(
        chat_id=chat_id,
        text=f"📤 Upload initialized by @{message.from_user.username or 'user'}",
